  ...
  if [[ ${COMP_CWORD} -eq 1  ]] ; then
    ...
    if [[ "${COMP_WORDS[@]}" == *" build"* || "${COMP_WORDS[@]}" == *" test"* ]] ; then
      if [[ "--start-with" == *${prev}* && ${cur} != -* ]] ; then
        ...
        COMPREPLY=($(compgen -W "--ament-cmake-args --build-space --build-tests -C --cmake-args --end-with --exclude --force-ament-cmake-configure --force-cmake-configure --make-flags --install-space --isolated --only-package --skip-build --skip-install --start-with --symlink-install" -- ${cur}))
      fi
