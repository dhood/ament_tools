...
def find_package_paths(basepath, exclude_patterns=None):
    """
    ...
    :param list exclude_patterns: A list of paths/regular expressions for paths
    ...
    if exclude_patterns is not None:
        real_exclude_paths = [
            ...
            if os.path.isdir(p)]
        exclude_patterns = [
            p for p in exclude_patterns if not os.path.isdir(p)]
        if exclude_patterns:
            exclude_pattern = re.compile('(?:%s)' % '|'.join(
                exclude_patterns))
    for dirpath, dirnames, filenames in os.walk(basepath, followlinks=True):
        real_dirpath = os.path.realpath(dirpath)
        if 'AMENT_IGNORE' in filenames \
                or real_dirpath in real_exclude_paths \
                or (exclude_pattern and exclude_pattern.match(real_dirpath)):
            del dirnames[:]
            continue
...
def find_packages(basepath, exclude_patterns=None):
    """
    ...
    package_paths = find_package_paths(
        basepath, exclude_patterns=exclude_patterns)
    for path in package_paths:
        packages[path] = parse_package(os.path.join(basepath, path))
    return packages


def find_unique_packages(basepath, exclude_patterns=None):
    """
    Crawl the filesystem to find package manifest files and parses them.
    ...
    """
    packages = find_packages(basepath, exclude_patterns=exclude_patterns)
